...
async def upload_document(
    ...
    async with DBConnection() as db:
        try:
            ...
            prev_docs = await db.fetch("SELECT document_address FROM is_documents WHERE is_number = $1 AND is_current = 1", is_number)
            for prev in prev_docs:
                await db.execute("UPDATE is_documents SET is_current = 0, superseded_by = $1 WHERE document_address = $2", doc_addr, prev["document_address"])
