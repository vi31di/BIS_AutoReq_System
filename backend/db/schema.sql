...
-- ParadeDB pg_search BM25 Index Creation
-- (Note: ParadeDB creates the indexes asynchronously using stored procedures)
SELECT pg_search.create_bm25(
  index_name => 'idx_tables_meta_search',
  table_name => 'tables_meta',
  text_fields => '{caption_text: {tokenizer: {type: "en"}, record_term_frequencies: true}}',
  json_fields => '{facets: {}}'
);

SELECT pg_search.create_bm25(
  index_name => 'idx_clauses_meta_search',
  table_name => 'clauses_meta',
  text_fields => '{heading_text: {tokenizer: {type: "en"}, record_term_frequencies: true}, body_text: {tokenizer: {type: "en"}, record_term_frequencies: true}}'
);
